...
class ArtworkTests(unittest.TestCase):
    def test_upstream_cover_precedes_logo_and_legacy_miss(self):
        from frontend.covers import upstream_art, cache_key
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            game = games.Game("Fixture", root / "Game")
            poster = root / "art.png"
            image = QImage(400, 600, QImage.Format.Format_RGB32)
            image.fill(QColor("red"))
            image.save(str(poster))
            (root / (cache_key(game) + ".missing-v3")).touch()
            with patch.object(upstream_art, "user", return_value={}), \
                 patch.object(upstream_art, "xbox", return_value={}), \
                 patch.object(upstream_art, "got", side_effect=[{}, {"logo": "missing", "cover": poster}]), \
                 patch.object(upstream_art, "lookup") as lookup, patch("frontend.covers.fetch") as fetch:
                result = load_cover(game, cache=root)
...
class LibraryTests(unittest.TestCase):
    ...
    def test_launch_feedback_and_duplicate_protection(self):
        w = self.window
        w._open_game(w.proxy.index(0, 0))
        self.drain()
        with patch("frontend.backend.autopilot.start", return_value=(False, "Fixture refusal")) as start:
            w.play_button.click()
            self.assertFalse(w.play_button.isEnabled())
            w.play_selected()
            self.drain()
            start.assert_called_once_with(w.current.game)
            self.assertIn("Fixture refusal", w.game_session_status.text())
            self.assertTrue(w.play_button.isEnabled())

    def test_active_inline_job_blocks_return_close_and_new_operation(self):
        from frontend.jobs import Jobs
        from PySide6.QtWidgets import QDialog
        w = self.window
        w._open_game(w.proxy.index(0, 0))
        self.drain()
        dialog = QDialog(w)
        dialog.jobs = Jobs(dialog)
        w._mount_game_tool(dialog)
        dialog.jobs.active.add(999)
        try:
            self.assertFalse(w._close_game_tool())
            self.assertIsNone(w._submit(lambda emit: None, busy=True))
            self.assertFalse(w.close())
            self.assertIs(w.inline_dialog, dialog)
        finally:
            dialog.jobs.active.clear()
            self.assertTrue(w._close_game_tool())

    def test_closed_game_opens_inline_diagnosis(self):
        from frontend.session import SessionResult
        w = self.window
        w._open_game(w.proxy.index(0, 0))
        self.drain()
        w.watch_check.setChecked(True)
        result = SessionResult("Fixture session", "fixture", "opti", verdict="Working.")
        with patch.object(w.service, "diagnose", return_value=result, create=True):
            w.watch_events.put(("closed", w.current.game.install_dir, 12))
            w._watch_tick()
            self.drain()
        self.assertEqual(w.game_reports[w.current.key], result)
